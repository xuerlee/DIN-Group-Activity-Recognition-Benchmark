...
class Basenet_new_new_collective(nn.Module):
    """
    main module of base model for collective dataset
    """
    ...
    def forward(self, batch_data):
        ...
        T = images_in.shape[1]  # num_frames but now its the groups number
        H, W = self.cfg.image_size
        OH, OW = self.cfg.out_size
        MAX_N = self.cfg.num_boxes
        NFB = self.cfg.num_features_boxes
        ...
        # Reshape the input data
        '''
        input images tensor: (B, T, 3, 480, 720)
        input bboxes tensor: (B, T, max_num_bboxes, 4)
        '''
